...
public final class SweepTask {
    ...
    public SweepTask(BedNoFireworks plugin) {
        this.plugin = plugin;
    }
    ...
    public int refill() {
        ...
        if (plugin.settings().sweepPlayers()) {
            for (Player player : plugin.getServer().getOnlinePlayers()) {
                plugin.cleaner().cleanPlayer(player);
            }
        }
        return added;
    }
    ...
    private void processBatch() {
        ...
        while (budget-- > 0) {
            ...
            try {
                plugin.cleaner().cleanChunk(chunk);
     ...
     * Используется при запуске плагина и по команде /nofireworks scan.
     *
