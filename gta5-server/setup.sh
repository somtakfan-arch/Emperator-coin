...
if [[ "$SKIP_GARAGE" != "1" ]]; then
    ...
    if fetch "$REPO_ZIP" "$TMP_DIR/repo.zip"; then
        ...
        for resource in phone_garage ls_character ls_inventory ls_shops ls_medical ls_tuning ls_rp ls_police ls_gangs ls_crime ls_property ls_interact ls_armoury ls_forum ls_turf ls_city ls_business ls_fuel ls_jobs ls_wild ls_race ls_smuggle ls_stash ls_places ls_owner ls_binds ls_world; do
...
# Топливо: заправки на карте и глохнущий на нуле двигатель.
ensure ls_fuel
# Легальные работы: маршруты по карте и метки работающих.
ensure ls_jobs
# Промыслы севера: карьер, лесопилка, охота, рыбалка.
ensure ls_wild
# Уличные гонки.
ensure ls_race
# Контрабанда морем и воздухом.
ensure ls_smuggle
# Семейные схроны. После ls_property: семьи держатся там.
ensure ls_stash
# Автосалон в мире и доски объявлений. После phone_garage и ls_forum:
# каталог и темы берутся оттуда.
ensure ls_places
# Владелец сервера: права и кошелёк. После ls_police и phone_garage.
ensure ls_owner
# Свои клавиши на действия. После ls_interact: каталог действий берётся оттуда.
ensure ls_binds
ensure ls_world
