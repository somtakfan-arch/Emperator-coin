...
class Storage:
    ...
    def seed_templates(self, catalog) -> None:
        """Insert catalog templates that don't exist yet (keeps minted counts)."""
        with self._connect() as conn:
            for t in catalog:
                conn.execute(
                    "INSERT OR IGNORE INTO nft_templates "
                    "(tid, name, series, rarity, emoji, kind, season, max_supply, minted, active) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 1)",
                    (t["tid"], t["name"], t["series"], t["rarity"], t["emoji"],
                     t["kind"], t["season"], max(0, t["max_supply"])))

    def nft_template(self, tid: str):
        with self._connect() as conn:
            r = conn.execute(
                "SELECT tid, name, series, rarity, emoji, kind, season, max_supply, minted, active "
                "FROM nft_templates WHERE tid=?", (tid,)).fetchone()
        if not r:
            return None
        return dict(zip(("tid", "name", "series", "rarity", "emoji", "kind", "season",
                         "max_supply", "minted", "active"), r))

    def nft_minted_map(self) -> dict:
        with self._connect() as conn:
            return {r[0]: r[1] for r in conn.execute("SELECT tid, minted FROM nft_templates").fetchall()}

    def nft_mint(self, tid: str, owner_id: int):
        """Mint one copy of a template to owner. Returns {item_id, tid, serial}
        or None if inactive/sold out/unknown."""
        now = int(time.time())
        with self._connect() as conn:
            r = conn.execute(
                "SELECT active, max_supply, minted FROM nft_templates WHERE tid=?", (tid,)).fetchone()
            if not r or not r[0]:
                return None
            active, maxs, minted = r
            if maxs > 0 and minted >= maxs:
                return None
            serial = minted + 1
            conn.execute("UPDATE nft_templates SET minted=minted+1 WHERE tid=?", (tid,))
            cur = conn.execute(
                "INSERT INTO nft_items (tid, owner_id, serial, acquired_at) VALUES (?, ?, ?, ?)",
                (tid, owner_id, serial, now))
            return {"item_id": cur.lastrowid, "tid": tid, "serial": serial}

    def nft_count(self, owner_id: int, kind: str = None) -> int:
        q = "SELECT COUNT(*) FROM nft_items i JOIN nft_templates t ON i.tid=t.tid WHERE i.owner_id=?"
        p = [owner_id]
        if kind:
            q += " AND t.kind=?"
            p.append(kind)
    ...
    def _nft_row(self, r):
        d = dict(zip(("item_id", "tid", "owner_id", "serial", "acquired_at", "transfers",
                      "xp", "level", "name", "series", "rarity", "emoji", "kind",
                      "max_supply", "minted"), r))
        return d

    def nft_items_page(self, owner_id: int, offset: int, limit: int, kind: str = None):
        q = ("SELECT i.item_id, i.tid, i.owner_id, i.serial, i.acquired_at, i.transfers, "
             "i.xp, i.level, t.name, t.series, t.rarity, t.emoji, t.kind, t.max_supply, t.minted "
             "FROM nft_items i JOIN nft_templates t ON i.tid=t.tid WHERE i.owner_id=?")
        p = [owner_id]
        if kind:
            q += " AND t.kind=?"
            p.append(kind)
        q += " ORDER BY t.rarity DESC, i.item_id LIMIT ? OFFSET ?"
        p += [limit, max(0, offset)]
        with self._connect() as conn:
            return [self._nft_row(r) for r in conn.execute(q, p).fetchall()]

    def nft_item(self, item_id: int):
        with self._connect() as conn:
            r = conn.execute(
                "SELECT i.item_id, i.tid, i.owner_id, i.serial, i.acquired_at, i.transfers, "
                "i.xp, i.level, t.name, t.series, t.rarity, t.emoji, t.kind, t.max_supply, t.minted "
                "FROM nft_items i JOIN nft_templates t ON i.tid=t.tid WHERE i.item_id=?",
                (item_id,)).fetchone()
            if not r:
                return None
            d = self._nft_row(r)
            m = conn.execute("SELECT price FROM nft_market WHERE item_id=?", (item_id,)).fetchone()
            d["price"] = m[0] if m else None
        return d

    def nft_owns_count_tid(self, owner_id: int, tid: str) -> int:
        with self._connect() as conn:
            return conn.execute("SELECT COUNT(*) FROM nft_items WHERE owner_id=? AND tid=?",
                                (owner_id, tid)).fetchone()[0]

    def nft_is_listed(self, item_id: int) -> bool:
        with self._connect() as conn:
            return conn.execute("SELECT 1 FROM nft_market WHERE item_id=?", (item_id,)).fetchone() is not None

    def nft_market_list(self, item_id: int, seller_id: int, price: int):
        with self._connect() as conn:
            it = conn.execute("SELECT owner_id FROM nft_items WHERE item_id=?", (item_id,)).fetchone()
            if not it or it[0] != seller_id:
                return False, "own"
            conn.execute(
                "INSERT INTO nft_market (item_id, seller_id, price, created_at) VALUES (?, ?, ?, ?) "
                "ON CONFLICT(item_id) DO UPDATE SET price=excluded.price",
                (item_id, seller_id, max(1, price), int(time.time())))
        return True, None

    def nft_market_unlist(self, item_id: int, seller_id: int) -> bool:
        with self._connect() as conn:
            cur = conn.execute("DELETE FROM nft_market WHERE item_id=? AND seller_id=?", (item_id, seller_id))
        return cur.rowcount > 0

    def nft_market_page(self, offset: int, limit: int, exclude=None):
        q = ("SELECT m.item_id, m.seller_id, m.price, i.tid, i.serial, t.name, t.rarity, t.emoji, t.series "
             "FROM nft_market m JOIN nft_items i ON m.item_id=i.item_id "
             "JOIN nft_templates t ON i.tid=t.tid")
        p = []
        ...
        with self._connect() as conn:
            rows = conn.execute(q, p).fetchall()
        return [dict(zip(("item_id", "seller_id", "price", "tid", "serial", "name",
                          "rarity", "emoji", "series"), r)) for r in rows]

    def nft_market_count(self, exclude=None) -> int:
        q = "SELECT COUNT(*) FROM nft_market"
        p = []
        if exclude is not None:
            q += " WHERE seller_id != ?"
            p.append(exclude)
    ...
    def nft_market_buy(self, item_id: int, buyer_id: int):
        now = int(time.time())
        with self._connect() as conn:
            m = conn.execute("SELECT seller_id, price FROM nft_market WHERE item_id=?", (item_id,)).fetchone()
            if not m:
                return False, "gone"
            seller, price = m
            if seller == buyer_id:
                return False, "own"
            it = conn.execute("SELECT owner_id FROM nft_items WHERE item_id=?", (item_id,)).fetchone()
            if not it or it[0] != seller:
                conn.execute("DELETE FROM nft_market WHERE item_id=?", (item_id,))
                return False, "gone"
            if not self.is_test_account(buyer_id):
                bal = conn.execute("SELECT balance FROM bed_balances WHERE user_id=?", (buyer_id,)).fetchone()
                if not bal or bal[0] < price:
                ...
                self._ledger(conn, buyer_id, -price, "nft_buy")
            self._credit(conn, seller, price)
            conn.execute("UPDATE nft_items SET owner_id=?, acquired_at=?, transfers=transfers+1 WHERE item_id=?",
                         (buyer_id, now, item_id))
            conn.execute("DELETE FROM nft_market WHERE item_id=?", (item_id,))
        return True, {"seller": seller, "price": price}

    def nft_gift(self, item_id: int, owner_id: int, target_id: int) -> bool:
        with self._connect() as conn:
            it = conn.execute("SELECT owner_id FROM nft_items WHERE item_id=?", (item_id,)).fetchone()
            if not it or it[0] != owner_id:
                return False
            if conn.execute("SELECT 1 FROM nft_market WHERE item_id=?", (item_id,)).fetchone():
                return False
            conn.execute("UPDATE nft_items SET owner_id=?, acquired_at=?, transfers=transfers+1 WHERE item_id=?",
                         (target_id, int(time.time()), item_id))
        return True

    def nft_burn(self, owner_id: int, item_ids):
        """Delete owned, not-listed items. Returns the count actually burned."""
        with self._connect() as conn:
            burned = 0
            for iid in item_ids:
                it = conn.execute("SELECT owner_id FROM nft_items WHERE item_id=?", (iid,)).fetchone()
                if not it or it[0] != owner_id:
                    continue
                if conn.execute("SELECT 1 FROM nft_market WHERE item_id=?", (iid,)).fetchone():
                    continue
                conn.execute("DELETE FROM nft_items WHERE item_id=?", (iid,))
                burned += 1
        return burned

    def nft_burn_fodder(self, owner_id: int, rarity: str, count: int):
        """Pick up to `count` burnable item_ids of a given rarity (owned, not
        listed). Returns the list."""
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT i.item_id FROM nft_items i JOIN nft_templates t ON i.tid=t.tid "
                "WHERE i.owner_id=? AND t.rarity=? AND i.item_id NOT IN (SELECT item_id FROM nft_market) "
                "ORDER BY i.item_id LIMIT ?", (owner_id, rarity, count)).fetchall()
        return [r[0] for r in rows]

    def nft_add_xp(self, item_id: int, owner_id: int, amount: int):
        """Feed a pet: +xp, recompute level (level = 1 + xp//100). Returns
        (ok, new_level, new_xp) or (False, None, None)."""
        with self._connect() as conn:
            r = conn.execute(
                "SELECT i.owner_id, i.xp, t.kind FROM nft_items i JOIN nft_templates t ON i.tid=t.tid "
                "WHERE i.item_id=?", (item_id,)).fetchone()
            if not r or r[0] != owner_id or r[2] != "pet":
                return False, None, None
            xp = r[1] + max(0, amount)
            lvl = 1 + xp // 100
            conn.execute("UPDATE nft_items SET xp=?, level=? WHERE item_id=?", (xp, lvl, item_id))
        return True, lvl, xp

    def nft_evolve(self, item_id: int, owner_id: int, new_tid: str) -> bool:
        """Swap a pet's template to its evolved form (serial preserved)."""
        with self._connect() as conn:
            it = conn.execute("SELECT owner_id FROM nft_items WHERE item_id=?", (item_id,)).fetchone()
            if not it or it[0] != owner_id:
                return False
            conn.execute("UPDATE nft_items SET tid=? WHERE item_id=?", (new_tid, item_id))
            conn.execute("UPDATE nft_templates SET minted=minted+1 WHERE tid=?", (new_tid,))
        return True

    def nft_series_progress(self, owner_id: int, catalog):
        """distinct owned tids per series vs total mintable tids per series."""
        with self._connect() as conn:
            owned = {r[0] for r in conn.execute(
                "SELECT DISTINCT tid FROM nft_items WHERE owner_id=?", (owner_id,)).fetchall()}
        from collections import defaultdict
        tot = defaultdict(int)
        have = defaultdict(int)
        for t in catalog:
            if t["max_supply"] < 0:
                continue  # hidden evolutions don't count toward a set
            tot[t["series"]] += 1
            if t["tid"] in owned:
                have[t["series"]] += 1
        return [{"series": s, "have": have[s], "total": tot[s], "done": have[s] == tot[s] and tot[s] > 0}
                for s in sorted(tot)]
