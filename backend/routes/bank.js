...
router.get('/me', (req, res) => {
  res.json({
    ...
    role: req.user.role,
    balance: models.getBalance(req.user.id),
  });
});

router.get('/transactions', (req, res) => {
  const limit = Math.min(Number(req.query.limit) || 100, 500);
  res.json(models.listTransactionsForUser(req.user.id, limit));
});
...
router.post('/transfer', (req, res) => {
  ...
  const recipient = models.findUserByUsername(toUsername);
  ...
  res.json({ balance: models.getBalance(req.user.id) });
});
...
router.post('/link-code', (req, res) => {
  ...
  models.createLinkCode(req.user.id, code, expiresAt);
