...
def render(
    *,
    ...
    b.para(
        "1.4. Материалы передаются Приобретателю в электронном виде путём направления ссылки "
        f"для скачивания на адрес электронной почты Приобретателя: {_fill(f.get('buyer_email'), 30)} "
        f"в срок не позднее {_fill(f.get('delivery_days'), 4)} календарных дней с момента "
        "поступления оплаты."
    )
    b.para(
    ...
    b.para(
        "2. Материалы переданы посредством направления ссылки для скачивания на адрес "
        f"электронной почты Приобретателя {_fill(f.get('buyer_email'), 24)}."
    )
